...
by geographical location (see [map](mapDWD.html) and `nearbyStations`), by temporal resolution (hourly, daily, monthly), 
...
From this, `metaIndex` and `geoIndex` are derived.
...
In that case it is really recommendable to [update R](https://github.com/brry/rhydro#install). 
...
File selection by station name/id and folder happens with [`selectDWD`](../html/selectDWD.html).
...
<!-- ``` -->
If you find `rdwd:::fileIndex` to be outdated (Error in download.file ... : cannot open URL),
...
`fileIndex` is created with the function `indexDWD` used in 
[meta.R](https://github.com/brry/rdwd/blob/master/R/meta.R#L205).
