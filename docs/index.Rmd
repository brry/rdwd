...
* website [source code and files](https://github.com/brry/rdwd/tree/master/docs)
...
If the reason for the error lies in FTP being blocked on your system, use html links instead:
```{r remove_ftp, eval=FALSE}
kl <- dataDWD(link, removeftp=TRUE)
```
Or look into the setting for the argument `method`.

## metaIndex

For a permanent curent metaIndex in `r helplink("nearbyStations")`, use:
```{r nearbyStations_mindex, eval=FALSE}
files <- indexFTP("hourly/sun", dir=tempdir())
cursun <- createIndex(files, dir=tempdir(), meta=TRUE)
head(cursun$metaIndex)
sunlinks <- nearbyStations(52.4, 13.1, radius=30, mindex=cursun$metaIndex)
View(sunlinks)
```
...
```{r readDWD_radar_RQ, echo=-1, eval=FALSE}
par(mar=c(2,2,2.2,5), mgp=c(3,0.7,0))
...
```
The rq folder is empty (2026-07-02)  but looked like this before:  
<img src="readDWD_radar_RQ-1.png" width="600">  
The `re/` files have dwdradar:::readHeader issues. Contact me if they need fixing.
...
## icon-eu
forecasts for europe also work with `r helplink("readDWD.grib2")`:
```{r, readDWD_grib2_iconeu, eval=FALSE}
...
# EUR <- terra::vect(paste0("/vsizip/vsicurl/https://gisco-services.ec.europa.eu/",
#                 "distribution/v2/nuts/shp/NUTS_RG_20M_2021_4326_LEVL_0.shp.zip"))
# terra::plot(EUR, add=TRUE)
```
plotting is currently [broken](https://github.com/rspatial/terra/issues/2113) on my system (2026-07-02) but would look like this:
<img src="readDWD_grib2_iconeu-1.png" width="600">
...
For others (e.g. .tar files in [readDWD.asc](https://brry.github.io/rdwd/raster-data.html#readdwd.asc) 
...
- Python module for radar data: [wradlib](https://github.com/wradlib/wradlib)
...
monthly <- tapply(kl$MO_TT.Lufttemperatur, format(kl$MESS_DATUM,"%m"), 
                  quantile, na.rm=TRUE, probs=0:10/10)
...
see [rainfall use case](https://brry.github.io/rdwd/use-case-rainfall-intensity-depends-on-temperature.html). 
...
[nearbyStations use case](https://brry.github.io/rdwd/use-case-plot-all-rainfall-values-around-a-given-point.html).  
