...
\normalsize
\vspace{11em}
Berry Boessenkool, e-Rum Milano, June 2020\\[1em]
...
\item FTP server somewhat difficult to search
...
\includegraphics[width=0.6\textwidth]{fig_extern/datasets_highlighted.PNG}
\end{frame}
...
\pause
\vspace{-1.6em}
<<radolan3, size="small", fig.width=6.5, out.height="16.5em", out.width="33em">>=
...
\begin{frame}[fragile]
\begin{textblock*}{25em}(25pt,50pt) % topleft corner x=10pt, y=30pt. width=18em
select, download + read data\\[0.5em]
...
\end{textblock*}
%
\begin{textblock*}{18em}(240pt, 35pt)
\includegraphics[width=8em]{../hex/hex.png}
\end{textblock*}
\vspace{10em}
