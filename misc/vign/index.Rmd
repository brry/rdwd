...
`r helplink("fileIndex")` for the entire server is created with the function 
`r helplink("indexFTP")` used in the last section of 
[rdwd-package.R](https://github.com/brry/rdwd/blob/master/R/rdwd-package.R#L249).  
To get an index for a certaiin subfolder, use:
...
For gridded data at `r helplink("gridbasebase")`, including 
...
# current index of available files for a given folder:
...
Besides `r helplink("dwdbase")` and `r helplink("gridbase")`, 
...
project_and_plot <- function(x, main1, main2, main3=NULL, ...)
  {
  par(mar=c(2,2,2.2,1), mgp=c(3,0.7,0))
  x <- projectRasterDWD(x, quiet=TRUE, ...)
  x <- x[[1]] # plot only the first layer, if applicable
  main4 <- paste(as.character(main2), as.character(main3))
  raster::plot(x, main=c(main1, main4), las=1)
  addBorders()
  return(invisible(x))
  }
```
...
In real life, you might not want to unzip to a temporary `exdir` 
and (at least when calling it for the first time) not use quiet=TRUE.
You can also remove `read=FALSE` in dataDWD and add the needed arguments right there, 
but I wanted to be explicit here.

The first line in each code block shows for which FTP folder this function will be called.  
...
file <- dataDWD(link, base=gridbase, joinbf=TRUE, dir=ddir, read=FALSE, quiet=TRUE)
rad <- readDWD(file, exdir=tdir, selection=1:3, quiet=TRUE) # with toraster=TRUE
project_and_plot(rad$data, ".binary SF", rad$meta$date[1])
```
...
file <- dataDWD(link, base=gridbase, joinbf=TRUE, dir=ddir, read=FALSE, quiet=TRUE)
rad <- readDWD(file, exdir=tdir, selection=1:3, quiet=TRUE)
project_and_plot(rad$data, ".binary RW", rad$meta$date[1], extent="rw")
```

## readDWD.radar
`r helplink("readDWD.radar")` - not yet on cran, hence see the [source code](https://github.com/brry/rdwd/blob/master/R/readDWD.R#L818) 
```{r readDWD_radar, echo=-1}
suppressPackageStartupMessages(library(R.utils)) 
links <- indexFTP("hourly/radolan/recent/bin", base=gridbase, dir=tdir, quiet=TRUE) # 0.04 MB
file <- dataDWD(links[773], base=gridbase, joinbf=TRUE, dir=tdir, read=FALSE, quiet=TRUE)
rad <- readDWD(file, quiet=TRUE)
project_and_plot(rad$dat, ".radar", rad$meta$date)
```
...
file <- dataDWD(link, base=gridbase, joinbf=TRUE, dir=ddir, read=FALSE, quiet=TRUE)
rad <- readDWD(file, quiet=TRUE) # with dividebyten=TRUE
rad <- readDWD(file, quiet=TRUE) # runs faster at second time due to skip=TRUE
project_and_plot(rad, ".raster", "", proj="seasonal", extent=rad@extent)
```
...
rad <- readDWD(file, selection=1:3, dividebyten=TRUE, quiet=TRUE)
project_and_plot(rad, ".asc", names(rad)[1])
```
 
## readDWD.nc
`r helplink("readDWD.nc")` - not yet on cran, hence see the [source code](https://github.com/brry/rdwd/blob/master/R/readDWD.R#L706)
```{r readDWD_nc}
link <- "daily/Project_TRY/pressure/PRED_199606_daymean.nc.gz"  #  5 MB
file <- dataDWD(link, base=gridbase, joinbf=TRUE, dir=ddir, read=FALSE, quiet=TRUE)
rad <- readDWD(file, quiet=TRUE) # can also have interactive selection of variable
e_berry  <- c(5.0, 15.9,  46.8, 55.5) # still figuring out projection info
project_and_plot(rad$var, ".nc", rad$varname, rad$time[1], extent=e_berry, latlon=FALSE)
```

Of all these, `readDWD.nc` is the most likely to change, so in your code, you might add
```{r}
if(packageVersion("rdwd") > "1.2") warning("readDWD.nc output might have changed by now!")
```
This is because it may call `raster` directly on the unzipped file in the future, hence returning a different output by default.
It may also need changes to accurately project the maps.
...
Note: I'm using `quiet=TRUE` to keep this vignette concise. In real life, the messages may be important to you...
...
precip_files <- dataDWD(index[6:8], base=gridbase, joinbf=TRUE, read=FALSE, 
                        dir="../localdata", quiet=TRUE)
precip <- readDWD(precip_files, quiet=TRUE)
```
...
precip_proj <- projectRasterDWD(precip[[1]], proj="seasonal", 
                                extent=precip[[1]]@extent, quiet=TRUE)
...
[line 47](https://github.com/brry/rdwd/blob/master/R/projectRasterDWD.R#L52).  
One user reported using `dwd.projection <- '+init=epsg:31467'` instead.
