...
```
```{r meta2}
meta_monthly_rain <- dataDWD(m_link, dir=locdir())
...
(called through `r helplink("plotRadar")`). 
...
## readDWD.grib2
`r helplink("readDWD.grib2")` - since version 1.4.12 (2021-01-01)
```{r readDWD_grib2, fig.width=6, fig.height=5}
nwpbase <- "ftp://opendata.dwd.de/weather/nwp/icon-d2/grib/00/t_2m"
links <- indexFTP("", base=nwpbase, dir=tdir)
file <- dataDWD(links[6], base=nwpbase, joinbf=TRUE, dir=tdir, read=FALSE)
#forecast <- readDWD(file)
#pp <- project_and_plot(forecast, ".grib2", Sys.Date(), project=FALSE)
```
...
file <- dataDWD(link, read=FALSE, dir=locdir(), force=NA)
...
```{r uc_histrecent}
histrecent <- function(station)
  {
  id <- findID(station, failempty=TRUE) # avoid id=NA download of all files
  kl <- rdwd::selectDWD(id=id, res="daily", var="kl", per="hr")
  if(length(kl) != 2) stop(station, " has ", length(kl), " files instead of 2.")
  kl <- rdwd::dataDWD(kl, dir=locdir(), force=NA)
  kl <- do.call(rbind, kl) # historical and recent data
  kl <- kl[!duplicated(kl$MESS_DATUM),] # keep only hist for overlap
  kl[,c("STATIONS_ID", "QN_3", "QN_4", "eor")] <- NULL # unneeded columns
  rownames(kl) <- NULL
  kl # output
  }
hr_pot <- histrecent("Potsdam")
hr_cel <- histrecent("Celle")
str(hr_pot)
```
...
precip <- dataDWD(index[6:8], base=gridbase, joinbf=TRUE, dir=locdir())
```
...
[line 62](https://github.com/brry/rdwd/blob/master/R/projectRasterDWD.R#L62).  
...
rw_file <- dataDWD(rw_urls[9], base=rw_base, joinbf=TRUE, dir=tempdir(), read=FALSE, dbin=TRUE)
...
kl <- dataDWD(url, varnames=TRUE, dir=locdir())
...
localfiles <- dataDWD(links[1:3], joinbf=TRUE, sleep=0.2, read=FALSE, dir=locdir())
```
...
```{r hourlyrain_map_static, message=FALSE, fig.height=2, fig.width=6, warning=FALSE}
library(OSMscale)
...
prec <- dataDWD(m$url, fread=TRUE, dir=locdir())
...
clims <- dataDWD(urls, varnames=FALSE, dir=locdir())
if(length(urls)==1) 
