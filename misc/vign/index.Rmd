...
```{r mindexupdate}
ll <- selectDWD("Potsdam", c("hourly","daily"), c("wind","kl"), "r", meta=TRUE, outvec=TRUE)[1:2]
ll <- sub(dwdbase, "", ll)
ll
...
file <- dataDWD(link, read=FALSE, dir=locdir(), force=NA, overwrite=TRUE)
...
links <- selectDWD(id=ids, res="daily", var="weather_phenomena", per="h", outvec=TRUE)
...
radfile <- "raa01-sf_10000-2107142350-dwd---bin.gz" 
