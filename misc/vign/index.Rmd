...
project_and_plot(rad$dat, ".radar RW", rad$meta$date)
```
...
links <- indexFTP("", base=rqbase, dir=tdir, quiet=TRUE) # 0.07 MB
file <- dataDWD(links[17], base=rqbase, joinbf=TRUE, dir=tdir, read=FALSE, quiet=TRUE)
rad <- readDWD(file, quiet=TRUE)
project_and_plot(rad$dat, ".radar RQ", rad$meta$date)
```
...
rw_file <- dataDWD(url, dir=tempdir(), read=FALSE, dbin=FALSE, quiet=TRUE)
...
rw_urls <- indexFTP(base=rw_base, dir=tempdir(), folder="", exclude.latest.bin=TRUE, 
                    quiet=TRUE)
rw_file <- dataDWD(rw_urls[8], base=rw_base, joinbf=TRUE, dir=tempdir(), read=FALSE, 
                   quiet=TRUE, dbin=TRUE)
