...
date: "build `r Sys.Date()` with rdwd version `r packageVersion('rdwd')`"
...
project_and_plot(rad, ".nc", rad@title, rad@z[[1]][1], proj="nc", extent="nc")
```
...
rw_urls <- indexFTP(base=rw_base, dir=tempdir(), folder="", exclude.latest.bin=TRUE, quiet=TRUE)
rw_file <- dataDWD(rw_urls[8], base=rw_base, joinbf=TRUE, dir=tempdir(), read=FALSE, quiet=TRUE, dbin=TRUE)
...
kohl_file <- dataDWD(base=phenocrop_base, file=kohl_url, joinbf=TRUE, 
                     dir=localtestdir(), read=FALSE, quiet=TRUE) 
